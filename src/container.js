...
eventBus.subscribe("PostCreated", new PostCreatedAnalyticsHandler(analyticsRepository, new CoreEventTranslator()));
eventBus.subscribe("PostLiked", new PostLikedAnalyticsHandler(analyticsRepository));
eventBus.subscribe("CommentAdded", new CommentAddedAnalyticsHandler(analyticsRepository));

const coreModule = new CoreModule({
  registerUserCommandHandler: new RegisterUserCommandHandler(userFactory, userRepository),
  loginUserCommandHandler: new LoginUserCommandHandler(userRepository, passwordHasher, tokenService),
  createPostCommandHandler: new CreatePostCommandHandler(postFactory, postRepository, notificationService, eventBus),
  addCommentCommandHandler: new AddCommentCommandHandler(postRepository, eventBus),
  likePostCommandHandler: new LikePostCommandHandler(postRepository, eventBus),
...
const analyticsModule = new AnalyticsModule({
  getAnalyticsSummaryQueryHandler: new GetAnalyticsSummaryQueryHandler(analyticsRepository),
});
